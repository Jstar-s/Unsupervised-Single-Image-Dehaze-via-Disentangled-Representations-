...
def main():
  ...
  parser = TrainOptions()
  opts = parser.parse()
  writer = SummaryWriter(log_dir='../logs/dehaze')
  # daita loader
  print('\n--- load dataset ---')
  ...
  for ep in range(ep0, opts.n_ep):
    for it, (images_a, images_b) in enumerate(train_loader):
      ...
      if (it + 1) % 2 != 0 and it != len(train_loader)-1:
        continue
      model.update_EG()
      writer.add_scalar("disA_loss", model.disA_loss, global_step=it)
      writer.add_scalar("dis_B_loss", model.disB_loss, global_step=it)
      writer.add_scalar("B_percp_loss", model.B_percp_loss, global_step=it)
      writer.add_scalar("Recon_IIloss", model.l1_recon_II_loss, global_step=it)
