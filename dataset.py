...
class dataset_unpair(data.Dataset):
  def __init__(self, opts):
    ...
    # B
    images_B = sorted(os.listdir(os.path.join(self.dataroot, opts.phase + 'B1')))
    self.B = [os.path.join(self.dataroot, opts.phase + 'B1', x) for x in images_B]
  ...
  def __getitem__(self, index):   
    if self.dataset_size == (self.A_size+10):
