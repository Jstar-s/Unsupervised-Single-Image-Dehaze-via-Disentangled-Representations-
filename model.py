...
class UID(nn.Module):
  ...
  def test_forward(self, image, a2b=True):
    if a2b:
        ...
        eps_b = self.get_z_random(std_b.size(0), std_b.size(1), 'gauss')
        self.z_attr_b = eps_b.mul(std_b).add_(self.mu_b)
        output = self.gen.forward_B(self.z_content, self.z_attr_b)
    return output
  ...
  def forward(self):
    ...
    """
    这里代码有问题，在原版的DRIT中为
    input_attr_forI = torch.cat((self.z_attr_i, self.z_attr_i, self.z_random),0)
    input_attr_forB = torch.cat((self.z_attr_b, self.z_attr_b, self.z_random),0)
    """
    ...
    """
    这里也有问题 
    self.fake_I_recon = self.gen.forward_D(self.z_content_recon_i, self.z_attr_recon_i)
    self.fake_B_recon = self.gen.forward_B(self.z_content_recon_b, self.z_attr_recon_b)
    """
  ...
  def backward_D(self, netD, real, fake):
    ...
    for it, (out_a, out_b) in enumerate(zip(pred_fake, pred_real)):
      ...
      # add sigimoid function make binary_cross_entrop work keep value between(0, 1)
      # all0 = torch.sigmoid(all0)
      # all1 = torch.sigmoid(all1)
  ...
  def backward_EG(self):
    ...
    # add new loss, DCLoss
    # print(self.fake_I_recon.shape)
    loss_dc = self.DCLoss((self.fake_I_recon +1)/2, 35) * 0.01
    loss_tv = self.TVloss(self.fake_I_recon) * 0.01
    loss_G = loss_G_GAN_I + loss_G_GAN_B + \
             loss_G_L1_II + loss_G_L1_BB + \
             loss_G_L1_I + loss_G_L1_B + \
             loss_kl_za_b + percp_loss_B + \
             percp_loss_I + loss_dc + \
             loss_tv
    """
    这里可以添加新的loss比如重建后的和原始的L1loss，或者ssimloss啥的，像这样一写在network里面
    """
